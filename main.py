...
MODEL_PATH = Path("models/model.joblib")              # 训练好的模型（网页预测复用，不提交 Git）
SCALER_PATH = Path("models/scaler.json")              # z-score 标准化参数（与模型配套）
...
def build_xy(rows, header):
    """返回 X（每行 = [1, 标准化数值..., one-hot 类别...]）、y（目标）、每行来源、标准化参数。

    注意：means/stds 必须保存下来，预测新样本时复用同一组（否则缩放不一致，预测会出错）。
    """
...
def main() -> int:
    ...
    X, y, src, means, stds = build_xy(rows, header)
    ...
    # 保存训练好的模型与标准化参数（供网页预测复用；models/ 已由 .gitignore 排除）
    if not HAVE_JOBLIB:
        print("\n未安装 joblib，跳过模型保存。请先运行：python -m pip install joblib")
        return 1
    MODEL_PATH.parent.mkdir(exist_ok=True)
    joblib.dump(model, MODEL_PATH)
    with open(SCALER_PATH, "w", encoding="utf-8") as f:
        json.dump({"means": means, "stds": stds}, f, ensure_ascii=False, indent=2)
    print(f"[保存] 模型已保存到 {MODEL_PATH}")
    print(f"[保存] 标准化参数已保存到 {SCALER_PATH}")
    return 0
